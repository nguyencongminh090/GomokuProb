...
void yxStaticJson()
{
    // Ensure thread state is sync
    Search::Threads.waitForIdle();
    // Use the main thread board for evaluation which is synced
    Search::Threads.main()->searchOptions = options;
    Search::Threads.main()->setBoardAndEvaluator(*board);
    Board &workingBoard = *Search::Threads.main()->board;

    // 1. Static Evaluation
    Value eval    = Evaluation::evaluate(workingBoard, options.rule);
    Value evalPOB = workingBoard.sideToMove() == BLACK ? eval : -eval;
    float winrate = Config::valueToWinRate(evalPOB);

    std::cout << "{\"static_eval\": " << evalPOB 
              << ", \"winrate\": " << std::fixed << std::setprecision(4) << winrate
              << ", \"patterns\": {\"black\": [";

    // 2. Patterns Black
    int size = workingBoard.size();
    for (int y = 0; y < size; ++y) {
        std::cout << "[";
        for (int x = 0; x < size; ++x) {
            Pos pos(x, y);
            // Use workingBoard to be safe, though global board should be same
            if (workingBoard.isInBoard(pos) && workingBoard.get(pos) == EMPTY) {
                 std::cout << (int)workingBoard.cell(pos).pattern4[BLACK];
            } else {
                 std::cout << "0";
            }
    ...
    for (int y = 0; y < size; ++y) {
        ...
        for (int x = 0; x < size; ++x) {
             Pos pos(x, y);
            if (workingBoard.isInBoard(pos) && workingBoard.get(pos) == EMPTY) {
                 std::cout << (int)workingBoard.cell(pos).pattern4[WHITE];
            } else {
                 std::cout << "0";
            }
            if (x < size - 1) std::cout << ",";
...
bool runProtocol()
{
    ...
    else if (cmd == "YXSTATICJSON")        CheckBoardOK(yxStaticJson);
