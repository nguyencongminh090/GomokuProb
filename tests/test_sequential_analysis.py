...
# ---------------------------------------------------------------------------
# 9. ProfileStore Layer 2 schema round-trip
# ---------------------------------------------------------------------------

class TestProfileStoreLayer2Schema(unittest.TestCase):

    def _make_store(self, tmp_path):
        from src.core.profile_store import ProfileStore
        return ProfileStore(db_path=tmp_path)

    def test_add_and_retrieve_with_layer2_fields(self):
        """
        Writing a record with Layer 2 fields and reading it back should
        preserve all values via get_all_games.
        """
        import tempfile
        ...
        from src.core.profile_store import ProfileStore
        store = ProfileStore(db_path=tmp_path)
        ...
        record = ProfileRecord(
            ...
            cac=0.38,
            p_cac=0.04,
            shannon_entropy=1.75,
            p_entropy=0.06,
            ensemble_score=0.60,
            fisher_chi2=19.5,
            layer2_verdict="Suspicious (Type B)",
        )
        row_id = store.add_game(record)
        self.assertGreater(row_id, 0)

        retrieved = store.get_all_games("testplayer", limit=1)
        self.assertEqual(len(retrieved), 1)
        rec = retrieved[0]

        self.assertAlmostEqual(rec.z_runs, 1.23, places=5)
        self.assertAlmostEqual(rec.rho1, -0.45, places=5)
        self.assertAlmostEqual(rec.cusum_max, 4.5, places=5)
        self.assertEqual(rec.change_point, 7)
        self.assertAlmostEqual(rec.cac, 0.38, places=5)
        self.assertAlmostEqual(rec.ensemble_score, 0.60, places=5)
        self.assertEqual(rec.layer2_verdict, "Suspicious (Type B)")

    def test_add_game_without_layer2_fields_ok(self):
        """
        Existing code that doesn't set Layer 2 fields (all None defaults)
        must still work — backward compatibility.
        """
        import tempfile
        from src.core.profile_store import ProfileRecord, ProfileStore
        ...
        row_id = store.add_game(record)
        self.assertGreater(row_id, 0)
        retrieved = store.get_all_games("oldplayer", limit=1)
        self.assertEqual(retrieved[0].layer2_verdict, None)
